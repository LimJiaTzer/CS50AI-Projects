    ...
    def best_future_reward(self, state):
        ...
        for action in available_actions:
            if (state, action) in self.q:
                maxi = max(maxi, self.q[(state, action)])
        return maxi
...
class MiniMaxAI():
    # ai is maximising and human is minimising
    def choose_action(self, game):
        if game.winner is not None:
            return game.winner 
    
        def minimax(game, alpha, beta):
            if game.winner is not None:
                return game.winner, None
            
            if game.player == 1:
                max_no = -10**9
                for action in Nim.available_actions(game.piles):
                    gamecopy = copy.deepcopy(game)
                    gamecopy.move(action)
                    no, _ = minimax(gamecopy, alpha, beta)
                    if no>max_no:
        ...
        _, optimal_move = minimax(game, float('-inf'), float('inf'))
        return optimal_move
...
def play(ai, human_player=None):
    ...
    while True:
        ...
        # Have AI make a move
            ...
            elif type(ai) == MiniMaxAI:
                pile, count = ai.choose_action(game)
